...
const loadBookingReceipt = async (docBooking, docUser) => {
    ...
    const pdfFormData = await fs.readFile("../../assets/Travel Receipt Template.pdf");
    ...
    preparedForNameField.setText(docUser.firstName + docUser.middleName + docUser.lastName);
...
const loadFlightTicket = async (docBooking, docUser, docFlight, isDepartureFlight) => {
    const pdfFormData = await fs.readFile("../../assets/plane-ticket-template-fillable.pdf");
    const pdfDoc = await PDFDocument.load(pdfFormData);
    const form = await pdfDoc.getForm();
    ...
    passengerNameField.setText(docUser.firstName + docUser.middleName + docUser.lastName);
    fromField.setText(docFlight.sourceAirportData.city + ", " + docFlight.sourceAirportData.country);
    toField.setText(docFlight.destAirportData.city + ", " + docFlight.destAirportData.country);
    dateField.setText(Intl.DateTimeFormat("en", {month: "2-digit", day: "2-digit"}).format(
    ...
    if (isDepartureFlight){
        ...
        seatField.setText(docBooking.departureFlight.seat.x + "," + docBooking.departureFlight.seat.y);
    } else {
        classField.setText(docBooking.returnFlight.classDescription);
        seatField.setText(docBooking.returnFlight.seat.x + "," + docBooking.returnFlight.seat.y);
    }
...
const sendBookingEmail = async (email, receipt, departureFlightTicket, returnFlightTicket) => {
    ...
    if (returnFlightTicket === null){
        let mailOptions = {
            ...
            text: "Hi, thank you for booking a flight with Air Toronto! Attached are your receipt for the flight booking" +
        ...
        let mailOptions = {
            ...
            text: "Hi, thank you for booking your flights with Air Toronto! Attached are your receipt for the flight booking" +
